    ...
    public class GenericTextFileProcessor
    {
        public static List<T> LoadFromTextFile<T>(string filePath) where T : class , new()
        {
            ...
            foreach(var row in lines)
            {
                ...
                // Splits row into individual columns. Now the index of this row matches
                // the index of header so the FirstName column header lines up with the FirstName
                // value in this row
                var vals = row.Split(',');

                // Loops through each header entry so we can compare that
                //against the list of columns from reflection. One we get the matching column
                // we can do the "SetValue" method to set the column value for our entry variable to the vals
        ...
        public static void SaveToTextFile<T> (List<T> data, string FilePath) where T : class
        {
            ...
            //removing last comma from the end first
